/**
 * DE/RU glossary term overview stubs + hreflang on EN glossary pages.
 */
const fs = require('fs');
...
function autoData(lang, slug, en) {
  const term = en.h1.replace(/\s*\|.*$/, '').trim();
  if (lang === 'de') {
    return {
      title: `${term} — Thailand Visum Glossar auf Deutsch`,
      h1: term,
      lede: `${en.desc.slice(0, 155)}${en.desc.length > 155 ? '…' : ''} Erklärung aus Pattaya auf Deutsch.`,
    };
  }
  return {
    title: `${term} — глоссарий виз Таиланд на русском`,
    h1: term,
    lede: `${en.desc.slice(0, 155)}${en.desc.length > 155 ? '…' : ''} Объяснение из Pattaya на русском.`,
  };
}
...
function build(lang, slug, data) {
  ...
  const labels =
    lang === 'de'
      ? { full: 'Vollständiger Glossar-Eintrag (EN)', consult: 'Beratung auf Deutsch', home: '/de/' }
      : { full: 'Полная статья глоссария (EN)', consult: 'Консультация на русском', home: '/ru/' };
  return `<!DOCTYPE html>
<html lang="${lang}">
<head>
<meta charset="UTF-8" />
...
<p><a href="/${SECTION}/${slug}/">${labels.full} →</a> · <a href="/glossary/">All glossary</a></p>
...
for (const slug of fs.readdirSync(path.join(ROOT, SECTION)).filter((d) => !SKIP.has(d) && fs.existsSync(path.join(ROOT, SECTION, d, 'index.html')))) {
  ...
  for (const lang of ['de', 'ru']) {
    ...
    report.terms.push(`${lang}/${SECTION}/${slug}`);
  }
